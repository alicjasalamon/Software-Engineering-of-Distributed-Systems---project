...
/**
 * Base class of repository of Application\Entity\Schedule document.
 */
abstract class ScheduleRepository extends \Mandango\Repository
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->documentClass = 'Application\Entity\Schedule';
        $this->isFile = false;
        $this->collectionName = 'application_entity_schedule';
    ...
    public function fixMissingReferences($documentsPerBatch = 1000)
    {
        $skip = 0;
        do {
            $cursor = $this->getCollection()->find(array('days' => array('$exists' => 1)), array('days' => 1))->limit($documentsPerBatch)->skip($skip);
            $ids = array_unique(array_reduce(
                array_values(array_map(function ($result) { return $result['days']; }, iterator_to_array($cursor)))
            , 'array_merge', array()));
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Application\Entity\Day')->getCollection();
                ...
                if ($idsDiff = array_diff($ids, $referenceIds)) {
                    $this->update(array(), array('$pull' => array('days' => array('$in' => $idsDiff))), array('multiple' => 1));
                }
            }
